package starling.utils
{
    ...
    public class AssetManager extends EventDispatcher
    {
        ...
        public function addTexture(name:String, texture:Texture):void
        {
			...
			if(mTextures[name] == texture){
				trace("ALREADY IN TEXTURES");
				trace(name+ " in mTextures already");
			}
			...
			//trace(" mTextures[name]: "+ mTextures[name]);
        }
        ...
        public function addTextureAtlas(name:String, atlas:TextureAtlas):void
        {
            ...
            mAtlases[name] = atlas;
			trace(" mAtlases[name]: "+ mAtlases[name]);
        }
        ...
        public function enqueue(...rawAssets):void
        {
            for each (var rawAsset:Object in rawAssets)
            {
                ...
                {
					trace("RAW ASSET 1: "+rawAsset);
					//Custom
					
				//	if(Loaded[getName(rawAsset)] == undefined){
		                    if (!rawAsset["exists"])
		                    {
								trace("NOT FOUND");
		...
		/*	trace("NAME NAME NAME: "+name);
			if(Loaded[name] != undefined){
				trace("LOADED DICT Loaded[name]: "+Loaded[name]);
				return name;
			}else{
				trace("LOAD NAME IS UNDEFINED");
				mQueue.push({
					name: name,
					asset: asset
				});
        ...
        private function loadRawAsset(name:String, rawAsset:Object, 
                                      onProgress:Function, onComplete:Function):void
        {
        /*   trace("RAW ASSET LOAD name: "+name);
       	    trace("RAW ASSET LOAD object: "+rawAsset);
			trace("mTextures: "+mTextures);
			trace("mAtlases: "+mAtlases);
			trace("mSounds: "+mSounds);
			trace("mXmls: "+mXmls);
			trace("mObjects: "+mObjects);
			trace("mByteArrays: "+Dictionary);
